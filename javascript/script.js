...
contactForm.addEventListener('submit', e => {
	...
	fetch('http://localhost/ulearn/review.php', {
		method: 'POST',
		body: JSON.stringify({
			name: name.value,
			email: email.value,
		...
		.then(response => {
			if ((response.status = 201)) {
				alert(
					'You have succesfully submitted a review to us 👍🏾👍🏾. We appreciate you efforts the growth of our brand 🤩😘'
				);
			} else {
				alert('Error 💣🎇!! Review not sent. Please check your inputs, we would love to hear from you!!')
			}
		})
